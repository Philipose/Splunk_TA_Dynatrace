[dynatrace_timeseries_metrics://<name>]
dynatrace_tenant = If you are using Dynatrace Managed, include the environment ID: https://<tenant>/e/<environment>. Add-on forces mandatory HTTPS
dynatrace_api_token = https://www.dynatrace.com/support/help/dynatrace-api/
dynatrace_collection_interval = Relative timeframe passed to Dynatrace API. Timeframe of data to be collected at each polling interval.
dynatrace_tags = Returns entities must have all provided tags (Seperated by ampersand)
dynatrace_entities = Returns entities must have any provided Dynatrace entity IDs (Seperated by ampersand)
ssl_certificate_verification = 

[dynatrace_timeseries_single_metric://<name>]
dynatrace_tenant = If you are using Dynatrace Managed, include the environment ID: https://<tenant>/e/<environment>. Add-on forces mandatory HTTPS
dynatrace_api_token = https://www.dynatrace.com/support/help/dynatrace-api/
dynatrace_collection_interval = Relative timeframe passed to Dynatrace API. Timeframe of data to be collected at each polling interval.
dynatrace_metric = https://www.dynatrace.com/support/help/dynatrace-api/timeseries/how-do-i-fetch-the-metrics-of-monitored-entities/#available-timeseries
aggregation_type = 
dynatrace_tags = Returns entities must have all provided tags (Seperated by ampersand)
dynatrace_entities = Returns entities must have any provided Dynatrace entity IDs (Seperated by ampersand)
ssl_certificate_verification = 

[dynatrace_problem://<name>]
dynatrace_tenant = If you are using Dynatrace Managed, include the environment ID: https://<tenant>/e/<environment>. Add-on forces mandatory HTTPS
dynatrace_api_token = https://www.dynatrace.com/support/help/dynatrace-api/
dynatrace_collection_interval = Relative timeframe passed to Dynatrace API. Timeframe of data to be collected at each polling interval.
dynatrace_tags = Returns entities must have all provided tags (Seperated by ampersand)
ssl_certificate_verification = 

[dynatrace_entity://<name>]
dynatrace_tenant = If you are using Dynatrace Managed, include the environment ID: https://<tenant>/e/<environment>. Add-on forces mandatory HTTPS
dynatrace_api_token = https://www.dynatrace.com/support/help/dynatrace-api/
dynatrace_collection_interval = Relative timeframe passed to Dynatrace API. Timeframe of data to be collected at each polling interval.
dynatrace_management_zone = Management Zone the entity belongs to
dynatrace_tags = Returns entities must have all provided tags (Seperated by ampersand)
dynatrace_entities = Returns entities must have any provided Dynatrace entity IDs (Seperated by ampersand)
entity_endpoints = 
ssl_certificate_verification =
